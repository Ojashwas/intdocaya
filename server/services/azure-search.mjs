...
export class AzureSearchAdapter {
  constructor({ endpoint, index, apiKey = '', managedIdentity = false }) {
    ...
    if (!apiKey && !managedIdentity)
      throw new Error('Azure AI Search requires an API key or managed identity.')
  ...
  async search(searchText, { top = 25, filter } = {}) {
    const result = await this.request(
      '/indexes/' + encodeURIComponent(this.index) + '/docs/search?api-version=2023-11-01',
      {
        search: searchText || '*',
        top,
        count: true,
